...
export default function SellPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { if (book) generate() }, [book, platform])
  ...
  return (
    <div>
      <TopBar
        ...
        back
        right={
          <button onClick={generate} disabled={loading} className="p-2 rounded-full hover:bg-[var(--cream-2)] transition-colors">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {PLATFORMS.map(p => (
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold flex-shrink-0 transition-all active:scale-95',
                  platform === p ? 'text-[var(--cream)]' : 'text-[var(--ink)] border border-[var(--line)]'
                )}
          ...
          <>
            ...
            <div className="p-4 rounded-2xl" style={{ background: 'var(--cream-2)' }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {listing.hashtags.map(tag => (
                  <span key={tag} className="px-2 py-1 rounded-lg text-xs font-medium" style={{ background: 'rgba(30,77,58,0.1)', color: 'var(--forest)' }}>
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                style={{ background: copied ? 'var(--success, #4A8B6F)' : 'var(--forest)', color: 'var(--cream)' }}
              >
                {copied ? <Check size={16} /> : <Copy size={16} />}
                {copied ? 'Copiato!' : 'Copia tutto'}
              </button>
              <button
                onClick={() => navigator.share?.({ title: listing.title, text: listing.description })}
                className="py-3 px-4 rounded-2xl flex items-center justify-center border transition-all active:scale-95"
